...
public final class RecipeTreeFavorites {
    ...
    public static Optional<ItemStack> bookmarkedCandidate(List<ItemStack> alternatives) {
        ...
        for (IElement<?> element : bookmarkList.getElements()) {
            Optional<IBookmark> bookmark = element.getBookmark();
            // A recipe bookmark also expresses that its display ingredient is
            // preferred. This matters when a bookmarked intermediate product
            // is represented by a recipe bookmark rather than an ingredient
            // bookmark (for example when JEI's BOOKMARKED sorter is enabled).
            if (bookmark.isEmpty()
    ...
    private static String typedIngredientKey(ITypedIngredient<?> typed) {
        ...
        Optional<net.minecraft.world.item.ItemStack> item = typed.getIngredient(VanillaTypes.ITEM_STACK);
    ...
    private static IElement<?> createRowBreak() {
        IElement<?> element = (IElement<?>) Proxy.newProxyInstance(
            ...
            (proxy, method, arguments) -> switch (method.getName()) {
                ...
                // The row-aware renderer consumes this marker as a blank slot
                // and advances to the next physical row. Keeping it hidden
                // prevents JEI's normal renderer from treating it as an item.
